 ...
 */
   ...
   *
   * @throws Exception|phpmailerException
   * @return bool
   */
  ...
  protected function MailSend($header, $body) {
    ...
    if ($this->SingleTo === TRUE && count($toArr) > 1) {
      foreach ($toArr as $val) {
        ...
        // implement call back function if it exists
        $isSent = ($rt == 1) ? 1 : 0;
        $this->doCallback($isSent, $val, $this->cc, $this->bcc, $this->Subject, $body, $this->From, $header, $params);
      }
    } else {
      $rt = $this->mail_passthru($to, $this->Subject, $body, $header, $params);
      // implement call back function if it exists
      $isSent = ($rt == 1) ? 1 : 0;
      $this->doCallback($isSent, $to, $this->cc, $this->bcc, $this->Subject, $body, $this->From, $header, $params);
    }
    if (isset($old_from)) {
...
//        $filename    = $attachment[1];
  ...
  */
   ...
   * @param string  $params
   */
  protected function doCallback($isSent, $to, $cc, $bcc, $subject, $body, $from = NULL, $header = NULL, $params = NULL) {
    if (!empty($this->action_function) && is_callable($this->action_function)) {
      $params = array($isSent, $to, $cc, $bcc, $subject, $body, $from, $header, $params);
